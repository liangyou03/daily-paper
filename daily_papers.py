...
ML-depth preference: Prefer ML load Medium or High whenever comparably relevant papers are available. READ TODAY and OPTIONAL METHOD PAPER should both have ML load Medium or High, and the full set should lean toward substantive modeling rather than descriptive biology alone. A Low-ML paper is acceptable only when it is an unusually strong biological anchor for the dissertation and no comparably relevant method-rich paper is available.
...
def _render_paper_metadata(p: dict) -> str:
    ...
    for item in p.get("data_types", []) or []:
        ...
        if label:
            data_tags.append(
                f'<span style="display:inline-block;background:#fffdfa;border:1px solid #d8cfc4;'
                f'border-radius:999px;padding:5px 9px;margin:0 5px 5px 0;color:#4b4742;'
                f'font-size:11px;line-height:1.35;">{label}</span>'
            )
...
def build_html(papers: list[dict], glossary: list[dict] | None = None,
    ...
    for number, p in enumerate(papers, start=1):
        ...
        paper_metadata = _render_paper_metadata(p)
        cards += f"""
